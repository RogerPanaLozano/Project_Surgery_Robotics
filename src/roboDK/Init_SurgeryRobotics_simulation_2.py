...
def move_robot(robot, gripper, needle, text_label):
    ...
    while True:
        ...
        if current_Servo_torques:
            t1 = current_Servo_torques.get("Torque_roll1")
            t2 = current_Servo_torques.get("Torque_roll2")
            t_pitch = current_Servo_torques.get("Torque_pitch")
            t_yaw = current_Servo_torques.get("Torque_yaw")
            servo_torques_msg = f"T1={round(t1)} T2={round(t2)} T3={round(t_pitch)} T4={round(t_yaw)}"
